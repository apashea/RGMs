...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if isfield(MDP,'MDP')
            ...
            % XXX_matlab-7: post-record F / Q.E sizes (paired call4 localization)
            global RGMS_XM7
            if ~isempty(RGMS_XM7) && isfield(RGMS_XM7,'on') && RGMS_XM7.on
                if ~isfield(RGMS_XM7,'n'), RGMS_XM7.n = 0; end
                RGMS_XM7.n = RGMS_XM7.n + 1;
                f_numel = numel(mdp.F);
                f_sum = sum(mdp.F(:));
                e_numel = 0;
                qf = NaN;
                if isfield(mdp,'Q') && isfield(mdp.Q,'E') && numel(mdp.Q.E) >= mdp.L
                    e_numel = numel(mdp.Q.E{mdp.L});
                end
                if isfield(mdp,'Q') && isfield(mdp.Q,'F')
                    qf = mdp.Q.F;
                end
                row = struct('n',RGMS_XM7.n,'t',t,'m',m,'L',mdp.L, ...
                    'path',rgms_xm7_path,'F_numel',f_numel,'F_sum',f_sum, ...
                    'QE_L_numel',e_numel,'Q_F',qf);
                if ~isfield(RGMS_XM7,'rows'), RGMS_XM7.rows = {}; end
                RGMS_XM7.rows{end+1} = row;
                if RGMS_XM7.n <= 4 || mod(RGMS_XM7.n,32) == 0
                    fprintf(1, '[XM7] n=%d t=%d m=%d L=%d path=%s F_numel=%d F_sum=%.6g QE_L_numel=%d Q_F=%.6g\n', ...
                        row.n, row.t, row.m, row.L, row.path, row.F_numel, row.F_sum, row.QE_L_numel, row.Q_F);
                end
            end
