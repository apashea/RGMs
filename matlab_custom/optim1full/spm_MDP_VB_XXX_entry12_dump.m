function [MDP] = spm_MDP_VB_XXX_entry12_dump(MDP,OPTIONS,dumpSpec)
%SPM_MDP_VB_XXX_ENTRY12_DUMP  OPTIM1FULL fork — extended boundary times t=10,20,30.
%
% Prepended on MATLAB path for call4 investigation only (``matlab_custom/optim1full/``).
% Canonical shared dump: ``matlab_custom/entry12/spm_MDP_VB_XXX_entry12_dump.m``.
...
% Fork of spm_MDP_VB_XXX.m for Entry 12 MATLAB capture — keep aligned with
% matlab_src toolbox original when not dumping.
%
...
% Optional Entry 12 checkpoint dumping (third argument)
%--------------------------------------------------------------------------
if nargin < 3 || isempty(dumpSpec)
...
if size(MDP,2) > 1
    ...
    for i = 1:size(MDP,2)                      % number of trials
        ...
        %------------------------------------------------------------------
        ds = dumpSpec;
        if dumpSpec.enabled && size(MDP,2) > 1
            ds.runTag = sprintf('%s_trial%02d', dumpSpec.runTag, i);
        end
        OUT(:,i) = spm_MDP_VB_XXX_entry12_dump(MDP(:,i),OPTIONS,ds);
        fprintf('Update %i\n',i);
...
% Entry 12 checkpoint: 12C (pre-time-loop)
%--------------------------------------------------------------------------
if dumpSpec.enabled
    c12 = struct();
    c12.M   = M;
...
% Entry 12 checkpoints: assembled PDP (12H) and spine metadata (12I)
%--------------------------------------------------------------------------
if dumpSpec.enabled
    PDP = MDP;
    entry12_dump_bundle_save(dumpSpec, '12H', OPTIONS, struct('subentry','12H'), struct('PDP', PDP));
    spine = struct('T', T, 'Nm', Nm, 'N', N, 'matlab_release', version);
    entry12_dump_bundle_save(dumpSpec, '12I', OPTIONS, struct('subentry','12I'), struct('spine', spine));
end
...
%ENTRY12_ASSIGN_T_BOUNDARY  Lean boundaries incl. OPTIM1FULL probe times 10, 20, 30.
if t == 1
    ws.out_t1 = snap;
elseif t == 2
    ws.out_t2 = snap;
...
if isempty(dumpSpec) || ~isfield(dumpSpec,'enabled') || ~dumpSpec.enabled
    return
end
if ~isfield(dumpSpec,'outDir') || ~isfield(dumpSpec,'runTag')
    return
end
if ~exist(dumpSpec.outDir, 'dir')
    mkdir(dumpSpec.outDir);
end
meta = struct();
meta.subentry = code;
meta.run_tag = dumpSpec.runTag;
meta.capture_instrument = 'spm_MDP_VB_XXX_entry12_dump.m';
meta.timestamp = datestr(now, 31);
if nargin >= 4 && ~isempty(meta_extra)
    for fn = fieldnames(meta_extra)'
        meta.(fn{1}) = meta_extra.(fn{1});
    end
end
S = bundle;
S.OPTIONS = OPTIONS;
