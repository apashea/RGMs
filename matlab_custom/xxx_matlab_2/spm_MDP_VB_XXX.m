...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if isfield(MDP,'MDP')
            ...
            % ===== XXX_matlab-2 probe: child shell =====
            global RGMS_XM2
            if isempty(RGMS_XM2), RGMS_XM2 = struct('n',0,'rows',{{}}); end
            RGMS_XM2.n = RGMS_XM2.n + 1;
            if RGMS_XM2.n <= 4
                row = struct();
                row.t = t; row.m = m; row.phase = 'shell';
                row.parent_has_Q = isfield(MDP(m),'Q');
                row.mdp_ptr = []; % filled after assign below via handle token on MDP field
                % identity of nested template vs returned later
                tok = containers.Map('UniformValues', false);
                if isstruct(mdp)
                    mdp.rgms_xm2_tok = tok;
                end
                row.shell_has_D = isfield(mdp,'D');
                row.shell_has_E = isfield(mdp,'E');
                row.shell_has_P = isfield(mdp,'P');
                row.shell_has_X = isfield(mdp,'X');
                if row.shell_has_D
                    row.D0 = mdp.D; % cell snapshot headers
                end
                if row.shell_has_E
                    row.E0 = mdp.E;
                end
                RGMS_XM2.rows{end+1} = row; %#ok<AGROW>
                fprintf(1, '[XM2] shell n=%d t=%g m=%g has_Q=%d has_D=%d has_E=%d has_P=%d has_X=%d\\n', ...
                    RGMS_XM2.n, t, m, row.parent_has_Q, row.shell_has_D, row.shell_has_E, row.shell_has_P, row.shell_has_X);
            end
            % ===== end XM2 shell =====

            % ===== XXX_matlab-2 reuse check =====
            global RGMS_XM2
            if ~isempty(RGMS_XM2) && RGMS_XM2.n >= 2 && RGMS_XM2.n <= 4
                row = struct();
                row.t = t; row.m = m; row.phase = 'reuse';
                row.n = RGMS_XM2.n;
                row.shell_has_persist = isfield(mdp,'rgms_xm2_persist');
                if row.shell_has_persist
                    row.persist_gen = mdp.rgms_xm2_persist('gen');
                    mdp = rmfield(mdp,'rgms_xm2_persist');
                else
                    row.persist_gen = NaN;
                end
                RGMS_XM2.rows{end+1} = row; %#ok<AGROW>
                fprintf(1, '[XM2] reuse n=%d t=%g m=%g has_persist_from_prev_wb=%d persist_gen=%g\\n', ...
                    RGMS_XM2.n, t, m, row.shell_has_persist, row.persist_gen);
            end
            % ===== end XM2 reuse =====
            ...
            % ===== XXX_matlab-2 probe: after MDP(m).MDP = mdp =====
            global RGMS_XM2
            if ~isempty(RGMS_XM2) && RGMS_XM2.n <= 4
                % find matching shell row for this n
                for ii = numel(RGMS_XM2.rows):-1:1
                    if isfield(RGMS_XM2.rows{ii},'phase') && strcmp(RGMS_XM2.rows{ii}.phase,'shell') ...
                            && RGMS_XM2.rows{ii}.t == t && RGMS_XM2.rows{ii}.m == m
                        r0 = RGMS_XM2.rows{ii};
                        break
                    end
                end
                row = struct();
                row.t = t; row.m = m; row.phase = 'after_wb';
                row.n = RGMS_XM2.n;
                % Does write-back replace nested template with returned mdp?
                row.parent_MDP_is_mdp = isequal(MDP(m).MDP, mdp);
                % Token survival: if shell put tok on mdp and nested returned same struct path
                row.returned_has_tok = isfield(mdp,'rgms_xm2_tok');
                if isfield(mdp,'rgms_xm2_tok')
                    mdp = rmfield(mdp,'rgms_xm2_tok');
                    MDP(m).MDP = mdp;
                end
                % D/E changed vs shell snapshot?
                if isfield(r0,'D0') && isfield(mdp,'D')
                    row.D_same_as_shell = isequal(mdp.D, r0.D0);
                else
                    row.D_same_as_shell = NaN;
                end
                if isfield(r0,'E0') && isfield(mdp,'E')
                    row.E_same_as_shell = isequal(mdp.E, r0.E0);
                else
                    row.E_same_as_shell = NaN;
                end
                % Next call reuse: store pointer token on parent MDP for next hit
                tok2 = containers.Map('UniformValues', false);
                tok2('gen') = RGMS_XM2.n;
                MDP(m).MDP.rgms_xm2_persist = tok2;
                RGMS_XM2.rows{end+1} = row; %#ok<AGROW>
                fprintf(1, '[XM2] after_wb n=%d t=%g m=%g MDP_eq_mdp=%d D_same=%g E_same=%g\\n', ...
                    RGMS_XM2.n, t, m, row.parent_MDP_is_mdp, row.D_same_as_shell, row.E_same_as_shell);
            end
            % ===== end XM2 wb =====
