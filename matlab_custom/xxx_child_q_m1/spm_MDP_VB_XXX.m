...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if isfield(MDP,'MDP')
            ...
            % ===== RGMS M1 Q-identity probe A (first parent-has-Q handoff) =====
            global RGMS_M1_QPROBE
            if isempty(RGMS_M1_QPROBE), RGMS_M1_QPROBE = struct('n',0,'done',false,'seen_no_Q',0); end
            if ~RGMS_M1_QPROBE.done
                if ~isfield(MDP(m),'Q')
                    RGMS_M1_QPROBE.seen_no_Q = RGMS_M1_QPROBE.seen_no_Q + 1;
                else
                    RGMS_M1_QPROBE.n = RGMS_M1_QPROBE.n + 1;
                    if RGMS_M1_QPROBE.n == 1
                        RGMS_M1_QPROBE.t = t; RGMS_M1_QPROBE.m = m;
                        RGMS_M1_QPROBE.A_seen_no_Q_before = RGMS_M1_QPROBE.seen_no_Q;
                        RGMS_M1_QPROBE.A_isequal = isequal(mdp.Q, MDP(m).Q);
                        tok = containers.Map('UniformValues', false);
                        MDP(m).Q.rgms_m1_tok = tok;
                        mdp.Q = MDP(m).Q;
                        mdp.Q.rgms_m1_tok('from_mdp') = 2;
                        RGMS_M1_QPROBE.A_mdp_map_write_visible_on_parent = isKey(MDP(m).Q.rgms_m1_tok, 'from_mdp');
                        MDP(m).Q.rgms_m1_tok('from_parent') = 3;
                        RGMS_M1_QPROBE.A_parent_map_write_visible_on_mdp = isKey(mdp.Q.rgms_m1_tok, 'from_parent');
                        if isfield(MDP(m).Q,'E')
                            RGMS_M1_QPROBE.A_E_snapshot = MDP(m).Q.E;
                            RGMS_M1_QPROBE.A_E_numel = numel(MDP(m).Q.E{mdp.L});
                        else
                            RGMS_M1_QPROBE.A_E_snapshot = [];
                            RGMS_M1_QPROBE.A_E_numel = 0;
                        end
                        if isfield(MDP(m).Q,'F')
                            RGMS_M1_QPROBE.A_F = MDP(m).Q.F;
                        else
                            RGMS_M1_QPROBE.A_F = NaN;
                        end
                        MDP(m).Q = rmfield(MDP(m).Q, 'rgms_m1_tok');
                        mdp.Q = MDP(m).Q;
                        RGMS_M1_QPROBE.phase = 'after_assign_before_nested';
                        fprintf(1, '[M1-Q] A first parent-has-Q handoff: t=%g m=%g prior_no_Q_hits=%g isequal=%d map_mdp2par=%d map_par2mdp=%d E_numel=%g F=%g\\n', ...
                            t, m, RGMS_M1_QPROBE.A_seen_no_Q_before, RGMS_M1_QPROBE.A_isequal, ...
                            RGMS_M1_QPROBE.A_mdp_map_write_visible_on_parent, ...
                            RGMS_M1_QPROBE.A_parent_map_write_visible_on_mdp, ...
                            RGMS_M1_QPROBE.A_E_numel, RGMS_M1_QPROBE.A_F);
                    end
                end
            end
            % ===== end M1 probe A =====
            ...
            % ===== RGMS M1 Q-identity probe B =====
            global RGMS_M1_QPROBE
            if ~isempty(RGMS_M1_QPROBE) && isfield(RGMS_M1_QPROBE,'phase') && strcmp(RGMS_M1_QPROBE.phase,'after_assign_before_nested') && ~RGMS_M1_QPROBE.done
                RGMS_M1_QPROBE.B_parent_has_Q = isfield(MDP(m),'Q');
                RGMS_M1_QPROBE.B_mdp_has_Q = isfield(mdp,'Q');
                if RGMS_M1_QPROBE.B_parent_has_Q && ~isempty(RGMS_M1_QPROBE.A_E_snapshot)
                    RGMS_M1_QPROBE.B_parent_E_unchanged = isequal(MDP(m).Q.E, RGMS_M1_QPROBE.A_E_snapshot);
                else
                    RGMS_M1_QPROBE.B_parent_E_unchanged = NaN;
                end
                if RGMS_M1_QPROBE.B_parent_has_Q && RGMS_M1_QPROBE.B_mdp_has_Q
                    RGMS_M1_QPROBE.B_isequal_Q = isequal(mdp.Q, MDP(m).Q);
                else
                    RGMS_M1_QPROBE.B_isequal_Q = NaN;
                end
                RGMS_M1_QPROBE.phase = 'after_nested_before_record';
                fprintf(1, '[M1-Q] B after nested: parent_E_unchanged=%g isequal_Q=%g\\n', ...
            ...
            % ===== RGMS M1 Q-identity probe C =====
            global RGMS_M1_QPROBE
            if ~isempty(RGMS_M1_QPROBE) && isfield(RGMS_M1_QPROBE,'phase') && strcmp(RGMS_M1_QPROBE.phase,'after_nested_before_record') && ~RGMS_M1_QPROBE.done
                RGMS_M1_QPROBE.C_isequal = isequal(MDP(m).Q, mdp.Q);
                tok = containers.Map('UniformValues', false);
                MDP(m).Q.rgms_m1_tok = tok;
                mdp.Q = MDP(m).Q;
                mdp.Q.rgms_m1_tok('after_wb') = 1;
                RGMS_M1_QPROBE.C_shared_via_handle = isKey(MDP(m).Q.rgms_m1_tok, 'after_wb');
                MDP(m).Q = rmfield(MDP(m).Q, 'rgms_m1_tok');
                mdp.Q = MDP(m).Q;
                MDP(m).Q = mdp.Q;
                if isfield(MDP(m).Q,'E')
                    RGMS_M1_QPROBE.C_E_numel = numel(MDP(m).Q.E{mdp.L});
                else
                    RGMS_M1_QPROBE.C_E_numel = 0;
                end
                if isfield(MDP(m).Q,'F')
                    RGMS_M1_QPROBE.C_F = MDP(m).Q.F;
                else
                    RGMS_M1_QPROBE.C_F = NaN;
                end
                RGMS_M1_QPROBE.done = true;
                RGMS_M1_QPROBE.phase = 'after_writeback';
                fprintf(1, '[M1-Q] C after writeback: isequal=%d shared_handle=%d E_numel=%g F=%g\\n', ...
