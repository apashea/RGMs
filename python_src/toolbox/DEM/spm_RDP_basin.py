"""Pass 1 transliteration of spm_RDP_basin.m."""
...
def spm_RDP_basin(MDP, S, chi, L=None):
    ...
    j = np.flatnonzero(r_keep)
    r_eye = sparse.eye(ns, ns, dtype=np.float64, format="csr")
    mdp = spm_RDP_compress(mdp, r_eye[:, j], "first")
