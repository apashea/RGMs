"""
OPTIM1 driver fork for DEM_AtariIII.m (non-visual path).

Same staged fence as ``DEM_AtariIII.py``; Entries **3, 4, 7, 8, 9, 10** use ``*_optim`` ledgers.
All other entries delegate to fidelity ``python_src`` ledgers unchanged.
"""
...
from python_src.toolbox.DEM.dem_atariiii_entry7 import assimilate_hit_miss_sequences as _entry7_assimilate_sequences  # noqa: F401 — fidelity ref
...
def _repo_root() -> Path:
    # Repo root is RGMs/, while this file lives in python_src/optimized/toolbox/DEM/.
...
def run_dem_atariiii_optim(
    entry_stop: int = 5,
    *,
    entry4_rgm_eig_pair=None,
    entry4_rgm_mi_override_fn=None,
    entry4_link_dir_mi_fn=None,
) -> dict[str, Any]:
    """
    Run OPTIM1 DEM_AtariIII driver entries up to `entry_stop` (implemented through Entry 12).

    Same contract as ``run_dem_atariiii``; Entries **3, 4, 7, 8, 9, 10** use optimized kernels.

    ``entry4_*`` hooks (keyword-only, default ``None``) are forwarded to Entry **4**
    ``spm_faster_structure_learning_optim`` → ``spm_rgm_group``. ``None`` keeps native
    compute unchanged for OPTIM1 / native lanes. OPTIM1FULL Product B parity passes
    MATLAB MI + ``eig(...,'nobalance')`` + link ``spm_dir_MI`` callables (see
    ``optim1full_entry4_matlab.py``).
    """
    if entry_stop < 1:
        raise ValueError("entry_stop must be >= 1")
    if entry_stop > 12:
        raise NotImplementedError("entry_stop > 12 is not implemented in DEM_AtariIII_optim.py")
    ...
    _rgms_run_set_last_label("run_dem_atariiii_optim: start")
    ...
    _rgms_run_set_last_label("ENTRY3: run_entry3_optim_from_boundary")
    _rgms_run_deadline_check()
    from python_src.toolbox.DEM.fsl_backward_entry3 import entry3_boundary_from_driver_ctx
    from python_src.optimized.toolbox.DEM.fsl_backward_entry3_optim import (
        run_entry3_optim_from_boundary,
    )

    out3 = run_entry3_optim_from_boundary(entry3_boundary_from_driver_ctx(ctx))
    ctx["GDP"] = out3["gdp"]
    ctx["PDP"] = out3["pdp"]
    # Keep generate PDP for native/COLAB1 gameplay fence (entry12 overwrites ctx['PDP']).
    ctx["PDP_generate"] = copy.deepcopy(out3["pdp"])
    ...
    out4 = run_entry4_optim_from_boundary(
        entry4_boundary_from_driver_ctx(ctx),
        rgm_eig_pair=entry4_rgm_eig_pair,
        rgm_mi_override_fn=entry4_rgm_mi_override_fn,
        link_dir_mi_fn=entry4_link_dir_mi_fn,
    )
    ctx["MDP"] = out4["mdp"]
    ...
    out7 = run_entry7_optim_from_boundary(entry7_boundary_from_driver_ctx(ctx))
    ctx["MDP"] = out7["mdp"]
    ...
    if entry_stop == 8:
        _rgms_run_set_last_label("ENTRY8: run_entry8_optim_from_boundary")
        _rgms_run_deadline_check()
        ...
        out8 = run_entry8_optim_from_boundary(entry8_boundary_from_driver_ctx(ctx, n_outer=n_outer))
        ctx["MDP"] = out8["mdp"]
    ...
    _rgms_run_set_last_label("ENTRY9: run_entry9_optim_from_boundary")
    _rgms_run_deadline_check()
    ...
    out9 = run_entry9_optim_from_boundary(entry9_boundary_from_driver_ctx(ctx, n_outer=n_outer))
    ctx["MDP"] = out9["mdp"]
    ctx["NS"] = out9["NS"]
    ctx["NU"] = out9["NU"]
    ctx["NA"] = out9["NA"]
    ...
    _rgms_run_set_last_label("ENTRY10: run_entry10_optim_from_mdp")
    _rgms_run_deadline_check()
    from python_src.optimized.toolbox.DEM.fsl_backward_entry10_optim import (
        run_entry10_optim_from_mdp,
    )

    out10 = run_entry10_optim_from_mdp(ctx["MDP"], c_val=float(ctx["C"]))
    ctx["MDP"] = out10["mdp"]
    ...
    # W3 / OPTIM1FULL native: same optim VB as parity (not fidelity).
    _rgms_run_set_last_label("ENTRY12: spm_MDP_VB_XXX_optim")
    _rgms_run_deadline_check()
    ctx["PDP"] = spm_MDP_VB_XXX_optim(ctx["RDP"])
    ...
    _rgms_run_set_last_label("run_dem_atariiii_optim: complete")
    _rgms_run_deadline_check()
    return ctx
