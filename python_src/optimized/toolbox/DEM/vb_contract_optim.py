...
def ind_prepare_bf_matvec(bf_prop: Any) -> Any:
    """Once-per-induction operator so the backward step is a stable CSR matvec.

    The backward propagation needs ``prev @ Bf`` at every step. When ``Bf`` is a
    SciPy sparse (CSR) matrix, ``vector @ csr`` forces SciPy to transpose and
    reconstruct the CSR on *every* step (profile: ``_csr.transpose`` /
    ``_compressed.__init__`` dominate ``ind_backward_paths_into``). Using the
    exact identity ``prev @ Bf == Bf.T @ prev`` we transpose ``Bf`` a single time
    here and reuse the CSR matvec for all goals and all steps in this induction
    call — matching MATLAB's stable ``Bf`` reuse in ``spm_induction`` (backwards
    loop ``any(Bf(I(:,n),:),1)``). Dense ``Bf`` already has no transpose tax, so
    ``None`` is returned and the dense ``np.dot`` fast path is used.
    """
    if bf_prop is None or isinstance(bf_prop, np.ndarray):
        return None
    return bf_prop.transpose().tocsr()
...
def ind_backward_paths_into(
    ...
    **C4f:** for sparse ``Bf`` pass ``bf_matvec = ind_prepare_bf_matvec(Bf)`` (``Bf.T`` CSR built
    once per induction call); the step becomes ``bf_matvec.dot(prev)`` — value-identical to
    ``prev @ Bf`` but without SciPy re-transposing/reconstructing the CSR every step.
    """
    if next_f is None:
    ...
    h = int(horizon_n)
    bf_work = bf_prop
    if bf_matvec is None and isinstance(bf_prop, np.ndarray) and bf_prop.dtype == bool:
        bf_work = bf_prop.astype(np.float64, copy=False)
    for n in range(h):
        prev = i_big[:, n]
        if not np.any(prev):
            break
        if bf_matvec is not None:
            next_f[:] = bf_matvec.dot(prev.astype(np.float64))
        elif isinstance(bf_work, np.ndarray):
