"""OPTIM1 Entry 8+9 — merge + ``spm_RDP_basin_optim`` outer loop."""
...
from python_src.optimized.toolbox.DEM.dem_atariiii_entry8_optim import _build_o_seg
...
def basin_training_loop(
    ...
    *,
    ...
    """ENTRY 8+9 optim ledger — merge optim then ``spm_RDP_basin_optim`` until ``all(d)``."""
    ...
    for i in range(1, int(n_outer) + 1):
        h.set_label(f"ENTRY9 optim: outer i={i}/{int(n_outer)}")
        h.deadline_check()
        ...
        h.set_label(f"ENTRY9 optim: spm_RDP_basin_optim outer i={i}/{int(n_outer)}")
        h.deadline_check()
        t_basin = time.perf_counter()
        mdp_out, d, o, hids, _ = spm_RDP_basin_optim(
