#!/usr/bin/env python3
"""FSL backward — Entry 4: compare Python ``MDP`` vs MATLAB authority.

**Authority:** ``fixtures/DEMAtariIII_fsl_backward_MDP_pre_entry10.mat`` variable
``MDP_pre_entry5`` (post Entry 4 structure learning on ``rng(2)`` ledger).

**Python:** ``fixtures/DEMAtariIII_fsl_backward_entry4_post.pkl`` field ``mdp``.

**Report:** ``matlab_custom/fsl_backward_compare_entry4_output.txt``
"""
...
def _report_path() -> Path:
    return _REPO / "matlab_custom" / "fsl_backward_compare_entry4_output.txt"
...
def _default_post_pkl() -> Path:
    ...
    return demo1_fixtures_dir() / "DEMAtariIII_fsl_backward_entry4_post.pkl"


def _default_authority_mat() -> Path:
    raw = str(os.getenv("RGMS_FSL_BACKWARD_ENTRY4_AUTHORITY_MAT_PATH", "")).strip()
    if raw:
        return Path(raw).expanduser().resolve()
    return (
...
def _load_mat_mdp(mat_path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if int(
            np.asarray(eng.eval("exist('MDP_pre_entry5','var')"), dtype=np.int64).reshape(-1)[0]
        ) != 1:
            raise KeyError(
                f"{mat_path} missing MDP_pre_entry5 — run patch_mdp_pre_entry5_to_pre_entry10_mat.m "
                "or dump_MDP_pre_entry10.m."
            )
        return _pull_mdp_from_matlab(eng, "MDP_pre_entry5")
    finally:
        eng.quit()
...
def _execute(args: argparse.Namespace) -> int:
    ...
    print(f"[FSL backward Entry 4] PKL post-Entry-4={pkl_path}", file=sys.stderr)
    print(
        f"[FSL backward Entry 4] MAT authority (MDP_pre_entry5)={mat_path}",
        file=sys.stderr,
    )
    print(
        f"[FSL backward Entry 4] levels py={len(py_mdp)} mat={len(mat_mdp)} Nm={blob.get('Nm')}",
        file=sys.stderr,
    )

    _assert_mdp_full_equal(py_mdp, mat_mdp, k=4)
    print("OK: MDP parity (FSL backward Entry 4)", file=sys.stderr)
    return 0


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="FSL backward Entry 4 MDP compare")
    ...
    header = (
        "FSL backward — Entry 4: compare Python post–Entry-4 ``MDP`` vs "
        "``MDP_pre_entry5`` authority.\n\n"
        f"**Report:** ``{report}``\n\n"
    )
