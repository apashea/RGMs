#!/usr/bin/env python3
"""FSL backward — Entry 7: compare Python post-Entry-7 ``MDP`` vs MATLAB authority.

**Authority:** ``fixtures/DEMAtariIII_fsl_backward_MDP_pre_entry10.mat`` variable
``MDP_pre_entry9`` (post Entry 7 hit/miss assimilations, pre Entry 8 loop).

**Python:** ``fixtures/DEMAtariIII_fsl_backward_entry7_post.pkl`` field ``mdp``.

**Report:** ``matlab_custom/fsl_backward_compare_entry7_output.txt``
"""
...
def _report_path() -> Path:
    return _REPO / "matlab_custom" / "fsl_backward_compare_entry7_output.txt"
...
def _default_post_pkl() -> Path:
    ...
    return demo1_fixtures_dir() / "DEMAtariIII_fsl_backward_entry7_post.pkl"


def _default_authority_mat() -> Path:
    raw = str(os.getenv("RGMS_FSL_BACKWARD_ENTRY7_AUTHORITY_MAT_PATH", "")).strip()
    if raw:
        return Path(raw).expanduser().resolve()
    return (
...
def _load_mat_mdp(mat_path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if int(
            np.asarray(eng.eval("exist('MDP_pre_entry9','var')"), dtype=np.int64).reshape(-1)[0]
        ) != 1:
            raise KeyError(
                f"{mat_path} missing MDP_pre_entry9 — re-run dump_MDP_pre_entry10.m."
            )
        return _pull_mdp_from_matlab(eng, "MDP_pre_entry9")
    finally:
        eng.quit()
...
def _execute(args: argparse.Namespace) -> int:
    ...
    if not mat_path.is_file():
        raise FileNotFoundError(
            f"missing authority mat: {mat_path}\nRun dump_MDP_pre_entry10.m (Entry 7 extension)."
        )
    ...
    print(f"[FSL backward Entry 7] PKL post-Entry-7={pkl_path}", file=sys.stderr)
    print(
        f"[FSL backward Entry 7] MAT authority (MDP_pre_entry9)={mat_path}",
        file=sys.stderr,
    )
    print(
        f"[FSL backward Entry 7] levels py={len(py_mdp)} mat={len(mat_mdp)} "
        f"n_windows={blob.get('n_windows')}",
        file=sys.stderr,
    )

    _assert_mdp_full_equal(py_mdp, mat_mdp, k=7)
    print("OK: MDP parity (FSL backward Entry 7)", file=sys.stderr)
    return 0


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="FSL backward Entry 7 MDP compare")
    p.add_argument("--pkl", type=Path, default=None, help="Python post-Entry-7 PKL")
    ...
    header = (
        "FSL backward — Entry 7: compare Python post-Entry-7 ``MDP`` vs "
        "``MDP_pre_entry9`` authority.\n\n"
        f"**Report:** ``{report}``\n\n"
    )
