#!/usr/bin/env python3
"""One-time: MATLAB ``.mat`` → PKL for FSL backward Entry 9 input.

Reads ``fixtures/DEMAtariIII_fsl_backward_MDP_pre_entry10.mat`` (from
``matlab_custom/fsl_backward/dump_MDP_pre_entry10.m``) — must include ``MDP_pre_entry9``,
``PDP_O``, ``NT`` (re-run dump after 2026-06 Entry 9 lane extension).

Writes ``fixtures/DEMAtariIII_fsl_backward_MDP_pre_entry9.pkl``.

Uses ``scipy.io.loadmat`` for ``PDP_O`` (fast); Engine pull for ``MDP_pre_entry9`` (struct fidelity).

Opt-in refresh: ``RGMS_FSL_BACKWARD_REFRESH_MDP_PRE9_PKL=1``
"""
...
def _refresh() -> bool:
    return str(os.getenv("RGMS_FSL_BACKWARD_REFRESH_MDP_PRE9_PKL", "")).strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def materialize_with_engine(eng: Any) -> dict[str, Any]:
    ...
    print(f"[FSL backward materialize Entry 9] loadmat PDP_O from {mat_p}", file=sys.stderr, flush=True)
    raw = loadmat(str(mat_p), simplify_cells=True)
    if "MDP_pre_entry9" not in raw:
        raise KeyError(
            f"{mat_p} missing MDP_pre_entry9 — re-run dump_MDP_pre_entry10.m (Entry 9 lane extension)."
        )
    if "PDP_O" not in raw:
    ...
    ne_arr = np.asarray(raw.get("Ne", [[2]]), dtype=np.float64).reshape(-1)
    nt_arr = np.asarray(raw.get("NT", [[100]]), dtype=np.float64).reshape(-1)
    meta_n_outer = 128
    meta = raw.get("meta")
    if isinstance(meta, np.ndarray) and meta.dtype.names and "n_outer" in meta.dtype.names:
        meta_n_outer = int(np.asarray(meta["n_outer"]).reshape(-1)[0])
    assert_pdp_o_columns_sufficient(
        pdp_o,
        ne=int(ne_arr[0]),
        nt=int(nt_arr[0]),
        n_outer=meta_n_outer,
    )
    print(
        f"[FSL backward materialize Entry 9] PDP_O shape rows={len(pdp_o)} cols={len(pdp_o[0]) if pdp_o else 0}",
        file=sys.stderr,
        flush=True,
    )

    eng.eval(f"load('{str(mat_p).replace(chr(92), '/')}');", nargout=0)
    print("[FSL backward materialize Entry 9] Engine pull MDP_pre_entry9", file=sys.stderr, flush=True)
    mdp = _pull_mdp_from_matlab(eng, "MDP_pre_entry9")
...
def main() -> int:
    ...
    if pkl.is_file() and not _refresh():
        print(f"[FSL backward materialize Entry 9] reuse {pkl}", file=sys.stderr)
        return 0
    ...
    print(f"[FSL backward materialize Entry 9] wrote {pkl}", file=sys.stderr)
    return 0
