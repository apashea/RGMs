#!/usr/bin/env python3
"""One-time: MATLAB ``.mat`` → reusable PKL for FSL backward Entry 11 input.

Reads ``fixtures/DEMAtariIII_fsl_backward_MDP_pre_entry11.mat`` (from
``matlab_custom/fsl_backward/dump_MDP_pre_entry11.m``) via MATLAB Engine pull.

Writes ``fixtures/DEMAtariIII_fsl_backward_MDP_pre_entry11.pkl`` (``mdp``, ``C``, ``meta``).

Re-run only when the ``.mat`` is refreshed. **Not** Entry 12; **not** full Python 1–11.

Opt-in refresh: ``RGMS_FSL_BACKWARD_REFRESH_MDP_PRE11_PKL=1``
"""
...
def _mat_path() -> Path:
    return _fixtures_dir() / "DEMAtariIII_fsl_backward_MDP_pre_entry11.mat"
...
def _refresh() -> bool:
    return str(os.getenv("RGMS_FSL_BACKWARD_REFRESH_MDP_PRE11_PKL", "")).strip().lower() in (
        "1",
        "true",
        "yes",
        "on",
...
def main() -> int:
    ...
    if pkl.is_file() and not _refresh():
        print(f"[FSL backward materialize] reuse {pkl}", file=sys.stderr)
        return 0
    ...
    print(f"[FSL backward materialize] wrote {pkl}", file=sys.stderr)
    return 0
