#!/usr/bin/env python3
"""OPTIM1FULL Product B — compare call-3 assembled RDP vs MATLAB on same ``MDP_post_nr``.

**Authority:** MATLAB assembly on ``DEMAtariIII_optim1full_MDP_post_nr.mat`` (Engine).
**Python:** ``DEMAtariIII_optim1full_call3_rdp.pkl``.
**Report:** ``matlab_custom/optim1full_compare_call3_rdp_output.txt``
"""
...
def _report_path() -> Path:
    return _REPO / "matlab_custom" / "optim1full_compare_call3_rdp_output.txt"
...
def _matlab_call3_rdp_from_post_nr_mat(mat_path: Path, *, c_val: float, ns: float) -> dict:
    ...
    try:
        ...
        tmp = _REPO / "matlab_custom" / "_optim1full_call3_rdp_ref.mat"
...
def _execute(args: argparse.Namespace) -> int:
    ...
    for label, path in (("PKL", pkl_path), ("MDP_post_nr mat", mat_path)):
        if not path.is_file():
            print(f"[OPTIM1FULL call3 compare] missing {label}: {path}", file=sys.stderr)
            return 2
    ...
    print(f"[OPTIM1FULL call3 compare] PKL={pkl_path}", file=sys.stderr)
    print(f"[OPTIM1FULL call3 compare] MAT={mat_path}", file=sys.stderr)

    rdp_mat = _matlab_call3_rdp_from_post_nr_mat(mat_path, c_val=c_val, ns=ns)
    py_vb = entry12_rdp_for_vb_from_mat_nested(rdp_py)
    mat_vb = entry12_rdp_for_vb_from_mat_nested(rdp_mat)
    _assert_nested_rdp_equal(py_vb, mat_vb, "OPTIM1FULL call3 VB-input RDP")
    print("[OPTIM1FULL call3 compare] PASS", file=sys.stderr)
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    p = argparse.ArgumentParser(description="OPTIM1FULL Product B call-3 RDP assembly compare")
    p.add_argument("--pkl", type=Path, default=None)
    p.add_argument("--mat", type=Path, default=None)
    args = p.parse_args(argv)
    if args.pkl is None:
        args.pkl = optim1full_call3_rdp_pkl()
    if args.mat is None:
