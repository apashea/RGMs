#!/usr/bin/env python3
"""OPTIM1FULL Product B — call-4 RDP assembly from ``MDP_post_nr`` (tier-2 analog).

Loads ``DEMAtariIII_optim1full_MDP_post_nr.mat``; runs Engine ``spm_RDP_sort``, then Python
``spm_RDP_MI`` / goals / costs / ``spm_mdp2rdp`` (``RGMS_OPTIM1FULL_SPM_RDP_SORT_MATLAB=1``).

This is the missing call-4 counterpart to ``optim1full_run_call3_assembly_isolated.py``.
Compare: ``optim1full_compare_call4_rdp_pkl_to_mat.py``.
"""
...
def main() -> int:
    ...
    if not optim1full_matlab_sort_enabled():
        print(
            "[OPTIM1FULL call4 asm] RGMS_OPTIM1FULL_SPM_RDP_SORT_MATLAB=0 — "
    ...
    if not pre.is_file():
        print(f"[OPTIM1FULL call4 asm] missing {pre}", file=sys.stderr)
        return 2

    print(f"[OPTIM1FULL call4 asm] input {pre}", file=sys.stderr)
    ...
    try:
        ...
        rdp = assemble_rdp_call4_post_nr_optim1full_parity(
            eng,
            mdp,
            c_val,
            ns,
        ...
        pickle.dump(
            {
                ...
                "ns": ns,
                "wall_s": wall_s,
                "boundary": "optim1full_call4_assembly",
                "validation": validation_sort_metadata(),
            },
            f,
            protocol=pickle.HIGHEST_PROTOCOL,
        )
    print(f"[OPTIM1FULL call4 asm] wrote {out} wall_s={wall_s:.3f}", file=sys.stderr)
    return 0
