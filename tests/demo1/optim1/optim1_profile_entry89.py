#!/usr/bin/env python3
"""OPTIM1 Iteration 1 — cProfile hot paths on DEMO1 ``pre_entry9`` boundary.

Profiles representative **single-call** merge and basin, plus one outer iteration
of the combined loop. Read-only on fidelity code; writes report under ``logs/``.

Usage (repo root)::

    python tests/demo1/optim1/optim1_profile_entry89.py
    python tests/demo1/optim1/optim1_profile_entry89.py --sort cumtime --top 40
"""
...
def _first_o_seg(boundary: dict[str, Any]) -> tuple[list, list[dict[str, Any]]]:
    """First inner merge window: outer ``i=1``, inner ``s=1``."""
...
def _profile_merge_once(boundary: dict[str, Any]) -> None:
    ...
    spm_merge_structure_learning(o_seg, mdp)
...
def _mdp_after_first_merge(boundary: dict[str, Any]) -> list:
    ...
    o_seg, mdp = _first_o_seg(boundary)
    return spm_merge_structure_learning(o_seg, mdp)
...
def _profile_basin_once(boundary: dict[str, Any]) -> None:
    ...
    spm_RDP_basin(mdp, [2, 3], [c_val, -c_val])
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="OPTIM1 Entry 8+9 cProfile on pre_entry9 boundary")
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Report path (default: logs/optim1_entry89_profile.txt)",
    )
    ...
    out_path = args.out or (_REPO / "logs" / "optim1_entry89_profile.txt")
    out_path.parent.mkdir(parents=True, exist_ok=True)

    sections: list[str] = [
        f"OPTIM1 Entry 8+9 profile report",
        f"pre_entry9={pre9}",
        f"Ne={boundary.get('Ne')} NT={boundary.get('NT')} n_outer={boundary.get('n_outer', 128)}",
        "",
    ]
    timings: dict[str, float] = {}
    profile_specs = [
        ("merge_once_s", "spm_merge_structure_learning (single call, first window)", lambda: _profile_merge_once(boundary)),
        ("basin_once_s", "spm_RDP_basin (single call, after first merge window)", lambda: _profile_basin_once(boundary)),
    ]
    ...
    if not args.skip_outer:
        print("[OPTIM1 profile] basin_training_loop n_outer=1", file=sys.stderr)
        text, elapsed = _run_profile(
            "basin_training_loop (n_outer=1)",
