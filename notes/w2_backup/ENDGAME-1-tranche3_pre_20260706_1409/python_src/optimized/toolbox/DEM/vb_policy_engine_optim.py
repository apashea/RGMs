...
_EfeSubcall = Callable[..., np.ndarray]
...
def policy_engine_step(
    ...
    *,
    ...
    H_slice = H[mi]
    P_now = [
        _fwd_col_vec(ws, ws_m, P, mi, f, t_col, use_ws=use_ws, bundle=bundle) for f in range(nf)
    ]
    if (
    ...
    for f in id_fp_list:
        ...
        Pf = _fwd_col_mat(ws, ws_m, P, mi, fi, t_col, use_ws=use_ws, bundle=bundle)
        Qp[fi] = Bf1 @ Pf

    pf_fu: dict[int, np.ndarray] = {}
    for f in id_fu_list:
        fi = int(f) - 1
        pf_fu[fi] = _fwd_col_mat(ws, ws_m, P, mi, fi, t_col, use_ws=use_ws, bundle=bundle)
    ...
    for f in id_iI_list:
        ...
        pmf_ii[fi] = _fwd_col_mat(
            ws, ws_m, P, mi, fi, t_col, use_ws=use_ws, bundle=bundle
        ).reshape(1, -1, order="F")
