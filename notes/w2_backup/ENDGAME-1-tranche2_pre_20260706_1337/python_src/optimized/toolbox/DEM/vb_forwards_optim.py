...
**Phase 6-C-2 (2026-07-05):** policy-loop ``spm_dot`` → ``vb_contract_optim`` (removed ``nq<=3`` cap).

**ENDGAME-1 tranche 1 (2026-07-06):** fix missing ``spm_softmax`` import on EFE path. (``Bf`` cache + batch ``Nk`` matmul tried — **reverted**; no call4 win; tranche 2 = unified policy loop.)
    ...
    *,
    ...
    for f in range(len(Q_upd)):
        if use_ws and ws is not None and ws_m is not None and Q_upd[f] is P_row[f]:
            ws_q_zero_inactive_tail(ws, ws_m, f, t - 1, None)
        else:
