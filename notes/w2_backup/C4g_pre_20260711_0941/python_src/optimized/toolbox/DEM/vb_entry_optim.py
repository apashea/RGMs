...
def _vb_child_mdp_checked(mdp_in: Any) -> list[dict[str, Any]]:
    ...
    Hierarchical prep shares parent ``Q`` (``XXX_matlab-1`` / ``.m`` ~1163–1165); checkX must not clone it.
    """
...
def _vb_run_child_pipeline(
    ...
    *,
    ...
    # One boundary sync: t-loop writes ``ws``; teardown Y/assemble read ``bundle`` Q/P/X.
