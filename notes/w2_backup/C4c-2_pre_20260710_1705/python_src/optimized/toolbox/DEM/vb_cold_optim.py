...
def vb_posterior_predictive_Y_child_optim(
    ...
    """MATLAB ~1591–1606 on nested child path — native ``forwards_dot_A_qj`` only.

    Do **not** call fidelity ``_entry12_probe_y_fill_all`` here: that fills
    ``entry12_Yfill`` (Entry 12 instrumentation, default-on env), not MATLAB PDP.
    Profile 2026-07-09: probe alone ~9 s cum on call4 child teardown (P9).
    ``3f`` may miss nested ``entry12_Yfill`` — labeled fidelity-gap; **-6** is value authority.
    """
