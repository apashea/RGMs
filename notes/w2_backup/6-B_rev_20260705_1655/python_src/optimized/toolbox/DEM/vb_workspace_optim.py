...
def ws_assign_q_belief_slot(
    ...
    """Write ``Q(m,f,t)`` — ``ws`` always; legacy slot when ``bundle`` given (**6-B** bridge)."""
    if bundle is not None:
        bundle["Q"][m][f][t] = col
...
def ws_assign_p_belief_slot(
    ...
    """Write ``P(m,f,t)`` — ``ws`` always; legacy slot when ``bundle`` given."""
    if bundle is not None:
        bundle["P"][m][f][t] = col
...
def ws_set_q_column(
    ...
    """Write ``Q(m,f,t)`` — ``ws`` always; legacy slot replace when ``bundle`` given."""
...
def ws_set_p_column(
    ...
    """Write ``P(m,f,t)`` — ``ws`` always; legacy slot replace when ``bundle`` given."""
    arr = np.asarray(values, dtype=np.float64)
    if bundle is not None:
        if arr.ndim == 1:
...
def ws_set_p_onehot(
    ...
    """``.m`` ~830–831: ``P{m,f,t}(:)=0; P{m,f,t}(u)=1``."""
...
def ws_copy_p_column(
    ...
    """Copy ``P(m,f,t_src)`` → ``P(m,f,t_dst)``."""
    if bundle is not None:
        src_slot = np.asarray(bundle["P"][m][f][t_src], dtype=np.float64).copy()
...
def ws_q_compact_column(
    ...
    """Compact ``Q(m,f,t)`` — active dim from ``Ns`` metadata (**6-B**)."""
...
def ws_p_compact_column(
    ...
    """Compact ``P(m,f,t)`` — active dim from ``Nu`` metadata."""
