...
**5-R-1b (2026-07-05):** belief slot-replace writes; compact ``ws`` reads in forwards/orchestrator.
"""
...
def _orch_assign_o_slot(
    ...
    """Write ``O(m,o,t)`` to ``ws`` only (**6-B** partial)."""
    ws = ws_require(bundle)
    ws_assign_o_slot(ws, None, mi, o_idx, t_idx, col)
...
def _prior_qp_paths_states_one_model(
    ...
    for f_idx in range(nf_gen):
        if f_idx < Um.size and float(Um[f_idx]) != 0.0:
            ...
            ws_set_p_onehot(ws, mi, f_idx, t_idx - 1, u_mark, bundle=bundle)

        nu_mf = int(Nu_m[mi, f_idx])
        Q_prev = ws_q_compact_column(ws, mi, f_idx, t_idx - 1, bundle).copy()
        Bmf = B_t[mi][f_idx]
        if nu_mf > 1:
            ...
            Q_new = Bm @ Q_prev
        ws_set_q_column(ws, mi, f_idx, t_idx, Q_new, bundle=bundle)
...
def _gen_control_one_model(mi: int, models: list[dict[str, Any]], bundle: dict[str, Any], t_idx: int) -> None:
    ...
    if float(bundle["process"][mi]) > 0.0:
        ...
        ws = ws_require(bundle)
        A_list = bundle["A"][mi]
        Q_slice = ws_q_row_at_t(ws, mi, t_idx, nf, bundle)
        nf_gp = len(md["GB"])
    ...
    for f_1 in fu:
        f_idx = int(f_1) - 1
        md["u"][f_idx, t_idx - 1] = float(_spm_sample(ws_p_cell(ws, mi, f_idx, t_idx - 1, bundle)))
        ...
        for g_idx in range(ng_loop):
            ...
            for o_1based in i_vals:
                ...
                if float(md["o"][o_idx, t_idx]) != 0.0:
                    if not _tensor_nonempty(ws_o_cell(ws, bundle, mi, o_idx, t_idx)):
                        no_mo = int(bundle["No"][mi, o_idx])
                        oi = int(round(float(md["o"][o_idx, t_idx])))
                        if no_mo > 0 and oi > 0 and oi <= no_mo:
                            _orch_assign_o_slot(
                                bundle, mi, o_idx, t_idx, _spm_one_hot(oi, no_mo)
                            )
                    continue
                ...
                if n_ot > 0:
                    ...
                    if ni == mi:
                        ...
                        q_list = [
                            ws_q_cell(ws, mi, int(jv) - 1, t_idx, bundle)
                            for jv in j_arr
                            if int(jv) > 0
                        ]
                        ...
                        _orch_assign_o_slot(bundle, mi, o_idx, t_idx, Ocell)
                        md["o"][o_idx, t_idx] = float(_spm_sample(Ocell))
                    else:
                        _orch_assign_o_slot(
                            bundle, mi, o_idx, t_idx, ws_o_cell(ws, bundle, ni, o_idx, t_idx)
                        )
                ...
                _orch_assign_o_slot(
                    ...
                    o_idx,
                    t_idx,
                    np.asarray(col, dtype=np.float64).reshape(-1, 1, order="F"),
                )
...
def vb_orchestrator_shared_probabilistic_outcomes(
    ...
    for mm in M_vec:
        ...
        for g_idx in range(ng_m):
            ...
            _orch_assign_o_slot(bundle, mi, g_idx, t_idx, O_dist)
...
def _fill_O_empty_from_realized_o(
    ...
    for o_idx in range(ng_m):
        if _tensor_nonempty(ws_o_cell(ws, bundle, mi, o_idx, t_idx)):
            continue
        if float(md["o"][o_idx, t_idx]) == 0.0:
            continue
        no_mo = int(bundle["No"][mi, o_idx])
        ...
        if no_mo > 0 and 0 < oi <= no_mo:
            _orch_assign_o_slot(bundle, mi, o_idx, t_idx, _spm_one_hot(oi, no_mo))
    ...
    for m in range(nm):
        ...
        for f_idx in range(nf_m):
            ...
            if controllable:
                ...
                Pmf_t = ws_p_cell(ws, m, f_idx, t_idx, bundle)
                if int(Nu[m, f_idx]) > 1:
...
def vb_orchestrator_belief_after_forwards(
    ...
    if t_m > 1:
        for f_idx in range(nf_m):
            ...
            if nu_mf > 1:
                ...
                ws_set_p_column(ws, mi, f_idx, t_idx - 1, post, bundle=bundle)
                logp = np.asarray(_spm_log(post), dtype=np.float64).reshape(-1, 1)
                Z_acc += float((post.T @ (LL + LP - logp)).reshape(-1)[0])
            else:
                ws_set_p_column(ws, mi, f_idx, t_idx - 1, np.array([1.0], dtype=np.float64), bundle=bundle)
    ...
    for f_idx in range(nf_m):
        if f_idx < Um_row.size and Um_row[f_idx] != 0.0:
            ...
            ws_set_p_column(ws, mi, f_idx, t_idx, col, bundle=bundle)
        else:
            if t_m > 1:
                ws_copy_p_column(ws, mi, f_idx, t_idx, t_idx - 1, bundle=bundle)
...
def vb_orchestrator_active_learning_in_loop(
    ...
    nf_m = int(bundle["Nf"][mi])
    ws = ws_require(bundle)
    Q_row: list[Any] = ws_q_row_at_t(ws, mi, t_idx, nf_m, bundle)
    ...
    if "a" in md:
        for g_1 in np.ravel(spm_children(id_m)).astype(np.int64):
            ...
            for i_out in k_flat:
                ...
                if io < 1:
                    continue
                ocell = ws_o_cell(ws, bundle, mi, io - 1, t_idx)
                if not _tensor_nonempty(ocell):
                    continue
    ...
    if "b" in md and t_m > 1:
        ws = ws_require(bundle)
        for f_idx in range(nf_m):
            Qt = ws_q_column(ws, mi, f_idx, t_idx, bundle)
            Qtm1 = ws_q_column(ws, mi, f_idx, t_idx - 1, bundle)
            Ptm1 = ws_p_column(ws, mi, f_idx, t_idx - 1, bundle)
            db = np.asarray(
...
def vb_orchestrator_in_loop_id_ig_and_sn(
    ...
    for f_idx in range(int(bundle["Nf"][mi])):
        ...
        for ii in range(t_int):
            q_src = ws_q_column(ws, mi, f_idx, ii, bundle)
            if ns <= 0:
                continue
