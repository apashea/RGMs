...
from python_src.optimized.toolbox.DEM.vb_cold_optim import (
    ...
    vb_cold_teardown_child_partial_native,
)
...
def _vb_child_mdp_checked(mdp_in: Any) -> list[dict[str, Any]]:
    ...
    ``Q`` isolation for child VB is **hierarchical prep** (fidelity ``~4239`` deepcopy until
    shared-``Q`` gate parity is proven against fidelity lane).
    """
...
def _vb_run_child_pipeline(
    ...
    *,
    ...
    """**5-C-arena** — lite nested child path: shared ``Q``, native child cold, lean **12F** only."""
    ...
    vb_cold_teardown_child_partial_native(models, bundle, opts, hp)
    if partial_ok:
