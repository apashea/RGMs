...
def _prior_qp_paths_states_one_model(
    ...
    for f_idx in range(nf_gen):
        if f_idx < Um.size and float(Um[f_idx]) != 0.0:
            ...
            ws_set_p_onehot(ws, mi, f_idx, t_idx - 1, u_mark)
        ...
        if nu_mf > 1:
            ...
            Q_new = Bm @ Q_prev
        ws_set_q_column(ws, mi, f_idx, t_idx, Q_new)
...
def vb_orchestrator_belief_after_forwards(
    ...
    if t_m > 1:
        for f_idx in range(nf_m):
            ...
            if nu_mf > 1:
                ...
                ws_set_p_column(ws, mi, f_idx, t_idx - 1, np.array([1.0], dtype=np.float64))
    ...
    for f_idx in range(nf_m):
        if f_idx < Um_row.size and Um_row[f_idx] != 0.0:
            ...
            ws_set_p_column(ws, mi, f_idx, t_idx, col)
        else:
            if t_m > 1:
                ws_copy_p_column(ws, mi, f_idx, t_idx, t_idx - 1)
