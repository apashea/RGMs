...
**0e (active):** nested shallow shell + ``Q`` deepcopy + in-place ``checkX`` — tests shared-``Q`` hypothesis.
...
import python_src.toolbox.DEM.spm_MDP_VB_XXX as _m
...
from python_src.optimized.toolbox.DEM.vb_workspace_optim import ws_from_bundle


def _vb_nested_entry_mdp_prepare(mdp_in: Any) -> Any:
    """
    Bisect **0e**: avoid full-tree ``deepcopy`` at nested VB entry.

    Hierarchical optim shares ``child["Q"] = parent["Q"]`` (``.m`` line 1165). Fidelity entry
    ``checkX(deepcopy(mdp_in))`` isolates ``Q`` during nested inference. **0a** skipped that clone
    entirely → ``Q.E`` drift. **0e** shallow-copies the dict shell and deepcopies ``Q`` only.
    """
...
def _vb_run_compute_pipeline(
    ...
    *,
    ...
    _m._vb_timing_set_band_wall("12A", time.perf_counter() - t_band)
    nm = len(models)
    ...
    _m._vb_timing_set_band_wall("12B", time.perf_counter() - t_band)
    if not lean and _m._vb_dump_active():
        _m._vb_dump_save(
            "12B",
            opts,
    ...
    _m._vb_timing_set_band_wall("12C", time.perf_counter() - t_band)
    if not lean and _m._vb_dump_active():
        _m._vb_dump_save(
            "12C",
            opts,
    ...
    _m._vb_timing_set_band_wall("12G", time.perf_counter() - t_band)
    t_band = time.perf_counter()
    vb_cold_assemble_12h(models, bundle)
    _m._vb_timing_set_band_wall("12H", time.perf_counter() - t_band)
    if partial_ok:
...
def run_optim_vb(
    ...
    **4-E-1** canonical optim VB entry — ``run_optim_vb(RDP, OPTIONS)``.
    ...
    """
    _m._vb_timing_enter()
    if _m._VB_TIMING_DEPTH == 1 and _m._vb_capture_y_probe_active():
        _m._ENTRY12_VBX_ACC = {}
    if _m._VB_TIMING_DEPTH == 1 and dump_subentries:
        _m._ENTRY12_PHASE_ACC = {}
    if monitoring and _m._VB_TIMING_DEPTH == 1:
        _m._VB_MONITOR_REQUESTED = True
    if dump_subentries and _m._VB_TIMING_DEPTH == 1:
        _m._VB_DUMP_SPEC = _m._vb_dump_resolve_spec()
    try:
        with VbRandContext(reuse_matlab_draws=reuse_matlab_draws):
            return _vb_run_compute_pipeline(
                mdp_in,
                options,
                nested=_m._VB_TIMING_DEPTH != 1,
                reuse_matlab_draws=reuse_matlab_draws,
                lean=False,
            )
    finally:
        _m._vb_timing_leave()
        if _m._VB_TIMING_DEPTH == 0:
            _m._VB_MONITOR_REQUESTED = False
            _m._VB_DUMP_SPEC = None
