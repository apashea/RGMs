...
from python_src.optimized.toolbox.DEM.vb_workspace_optim import ws_get, ws_pull_model_q_at_t
...
def _vb_run_partial_t_loop_optim(
    ...
    *,
    ...
    if _vb_mod._vb_dump_active():
        bundle["entry12_D"] = {
            "in": _vb_mod._entry12_snap_12d(models, bundle, 0, M_upd[0, :]),
        }
        bundle["entry12_E"] = {
            "in": _vb_mod._entry12_snap_12e(models, bundle, 0),
        }
        bundle["entry12_F"] = {
            "in": _vb_mod._entry12_snap_12f(models, bundle, 0, include_policy_traces=False),
        }
    for t_idx in range(t_int):
        ...
        if _vb_mod._vb_dump_active():
            _vb_mod._entry12_assign_t_boundary(
                bundle["entry12_D"],
                _vb_mod._entry12_snap_12d(models, bundle, t_1based, row),
                t_1based,
                t_int,
            )
        ...
        if _vb_mod._vb_dump_active():
            for mm in np.asarray(row, dtype=np.int64).ravel():
                mi_s = int(mm) - 1
                if mi_s >= 0:
                    _vb_mod._entry12_record_phase(mi_s, t_idx + 1, "post_share", bundle)
        ...
        if _vb_mod._vb_dump_active():
            snap_e = _vb_mod._entry12_snap_12e(models, bundle, t_1based, t_idx=t_idx)
            _vb_mod._entry12_attach_phase_log_to_snap(snap_e, bundle, t_1based, row)
            _vb_mod._entry12_assign_t_boundary(
                bundle["entry12_E"],
                snap_e,
                t_1based,
        ...
        for mm in np.asarray(row, dtype=np.int64).ravel():
            ...
            idm = bundle["id"][mi]
            if _vb_mod._vb_dump_active():
                ex: dict[str, Any] = {"A_peaks": _vb_mod._entry12_a_peaks_for_model(bundle["A"], mi)}
                for key in ("fp", "fu", "iH"):
                    if key in idm:
                        ex[f"id_{key}"] = np.asarray(idm[key], dtype=np.int64).ravel().tolist()
                _vb_mod._entry12_record_phase(mi, t_m, "pre_forwards", bundle, extra=ex)
            ...
            if _vb_mod._vb_dump_active():
                _vb_mod._entry12_record_phase(
                    mi, t_m, "post_forwards", bundle, extra={"F_after_fwd": float(F_elbo)}
                )
            ws = ws_fwd
            if ws is not None:
                ws_pull_model_q_at_t(bundle, ws, mi, t_idx)
            _vb_mod._entry12_attach_vbx_to_model(models, mi, t_m)
            ...
            if _vb_mod._vb_monitoring_active():
                t_1based = t_idx + 1
                if t_1based == 1:
                    _vb_mod._vb_monitor_snapshot("12F", models[mi], mi + 1, t_1based, "first")
                if t_1based == t_int:
                    _vb_mod._vb_monitor_snapshot("12F", models[mi], mi + 1, t_1based, "last")

        if _vb_mod._vb_dump_active():
            snap_f = _vb_mod._entry12_snap_12f(models, bundle, t_1based, include_policy_traces=True)
            _vb_mod._entry12_attach_phase_log_to_snap(snap_f, bundle, t_1based, row)
            _vb_mod._entry12_assign_t_boundary(
                bundle["entry12_F"],
                snap_f,
                t_1based,
        ...
        _vb_mod._vb_timing_add_12f(time.perf_counter() - t_iter)
