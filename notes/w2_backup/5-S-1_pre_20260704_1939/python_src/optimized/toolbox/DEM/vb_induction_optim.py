...
from python_src.toolbox.DEM.spm_MDP_VB_XXX import _numel
...
def _ind_backward_paths_into(
    ...
    """In-place backwards bool propagation into preallocated ``I_big``."""
    l_dim, ncols = I_big.shape
    I_big.fill(False)
    I_big[:, 0] = pf_col
    use_matvec = not isinstance(Bf_prop, sparse.spmatrix)
    for n in range(N):
        prev = I_big[:, n]
        if not np.any(prev):
            break
        if use_matvec:
            ...
            prev_f[:] = prev
            nxt = np.asarray(prev_f @ Bf_prop > 0).ravel()
        else:
            rows = np.flatnonzero(prev)
            nxt = np.asarray(Bf_prop[rows, :].sum(axis=0).A1 > 0).ravel()
        I_big[:, n + 1] = nxt
...
def _spm_induction_vb_optim(
    ...
    *,
    ...
    from python_src.toolbox.DEM import spm_MDP_VB_XXX as _vb_mod

    _PROBE_12F_PARENT = _vb_mod._PROBE_12F_PARENT
    ...
    ns_by_pos: list[int] = [int(B[f - 1][0].shape[0]) for f in hif_kept]
    ...
    if use_dense_bf:
        ...
        if d_flat is None:
            ...
            if d_mul.size != bf_nelem:
                raise ValueError("spm_induction: D size mismatch with Bf")
        Bf_prop: Any = _ind_bf_apply_d_mask_dense(Bf_dense_bool, d_flat, d_mul)
    else:
    ...
    prev_f = np.empty(l_dim, dtype=np.float64) if not isinstance(Bf_prop, sparse.spmatrix) else None
