...
**Phase 4-F-1 (2026-07-04):** ``_forwards_ws`` — primary t-loop path; ``ws`` column reads/writes via ``ws_set_q_column``.
...
def _efe_memo_key(
    ...
    """Cache key for recursive EFE ``spm_forwards`` subproblems."""
    qp_parts: list[bytes] = []
    for f in id_fu_list:
        fi_idx = int(f) - 1
        qv = np.asarray(qp[fi_idx], dtype=np.float64).ravel(order="F")
        qp_parts.append(qv.tobytes())
    return (int(m), int(t_next), int(k), tuple(int(x) for x in fi.tolist()), tuple(qp_parts))
...
def _fwd_col_vec(
    ...
    *,
    ...
    if use_ws:
        ...
        return np.asarray(ws.Q[ws_m][f][:, t_col], dtype=np.float64).reshape(-1, order="F")
...
def _fwd_col_mat(
    ...
    *,
    ...
    return _fwd_col_vec(ws, ws_m, P, mi, f, t_col, use_ws=use_ws).reshape(-1, 1, order="F")
...
def _fwd_write_belief_col(
    ...
    *,
    ...
    if use_ws:
        ...
        n = int(flat.size)
        ws.Q[ws_m][f][:, t_col] = 0.0
        if n:
            dim = int(ws.Q[ws_m][f].shape[0])
            ws.Q[ws_m][f][: min(n, dim), t_col] = flat[:dim]
...
def _forwards_compute(
    ...
    pA: list[Any],
    qa: Any | None,
    _efe_memo: dict[tuple[Any, ...], np.ndarray] | None,
    *,
    ws: VbWorkspace | None,
    ws_m: int | None,
    bundle: dict[str, Any] | None,
    ...
    A_row = A[mi]
    if _vb_mod._vb_dump_active():
        _vb_mod._entry12_record_phase_belief_rows(
            mi,
            t,
            "pre_vbx",
            O,
            P,
            P_row,
            extra={"A_peaks": _vb_mod._entry12_a_peaks_for_model(A, mi)},
        )
    if os.getenv("RGMS_ENTRY12_PROBE_CHILD_F") and int(t) == 1 and len(O[mi]) >= 100:
        import sys as _sys_child_f

        if not _vb_mod._ENTRY12_PROBE_CHILD_F_NPZ_DONE:
            import copy as _copy_probe
            import pickle as _pickle_probe

            _vb_mod._ENTRY12_PROBE_CHILD_F_NPZ_DONE = True  # noqa: PLW0603
    ...
    if _vb_mod._vb_capture_y_probe_active():
        _vb_mod._entry12_record_vbx_probe(mi, t, Q_upd, O_row, P_row, idm, F_vbx=F_vbx_here)
    for f in range(len(Q_upd)):
        _fwd_write_belief_col(ws, ws_m, bundle, P, mi, f, t - 1, Q_upd[f], use_ws=use_ws)
    ...
    B_slice = B[mi]
    H_slice = H[mi]
    P_now = [_fwd_col_vec(ws, ws_m, P, mi, f, t - 1, use_ws=use_ws) for f in range(nf)]
    if (
        os.getenv("RGMS_PROBE_12F_PARENT_T1")
        and t == 1
        and m == 1
        and nk >= 6
        and _vb_mod._PROBE_12F_PARENT is None
    ):
        _vb_mod._PROBE_12F_PARENT = {}
    ...
    if (
        ...
        and m == 1
        and nk >= 6
        and isinstance(_vb_mod._PROBE_12F_PARENT, dict)
    ):
        Rv0 = np.asarray(R, dtype=np.float64).ravel(order="F")
        _vb_mod._PROBE_12F_PARENT["R_sum_post_induction"] = float(np.sum(Rv0))
        _vb_mod._PROBE_12F_PARENT["R_nz_post_induction"] = np.flatnonzero(Rv0 > 0.0).tolist()[:8]
    ...
    for f in id_fp_list:
        ...
        Pf = _fwd_col_mat(ws, ws_m, P, mi, fi, t - 1, use_ws=use_ws)
        Qp[fi] = Bf1 @ Pf
    ...
    for f in id_fu_list:
        ...
        pf_fu[fi] = _fwd_col_mat(ws, ws_m, P, mi, fi, t - 1, use_ws=use_ws)

    log_hf: dict[int, np.ndarray] = {}
    for f in id_iH_list:
        fi = int(f) - 1
        Hf = np.asarray(H_slice[fi], dtype=np.float64).reshape(-1, 1, order="F")
        log_hf[fi] = _vb_mod._spm_log(Hf)

    pmf_ii: dict[int, np.ndarray] = {}
    for f in id_iI_list:
        fi = int(f) - 1
        pmf_ii[fi] = _fwd_col_mat(ws, ws_m, P, mi, fi, t - 1, use_ws=use_ws).reshape(1, -1, order="F")
    ...
    _probe_parent = bool(
        ...
        and m == 1
        and nk >= 6
        and _vb_mod._PROBE_12F_PARENT is not None
    )
    ...
    for k in range(nk):
        ...
        if _probe_parent and k == 0 and "G_before_iH" not in _vb_mod._PROBE_12F_PARENT:
            _vb_mod._PROBE_12F_PARENT["G_before_iH"] = float(np.asarray(G[k, 0], dtype=np.float64))

        for f in id_iH_list:
            fi = int(f) - 1
            Qf = np.asarray(Qp[fi], dtype=np.float64).reshape(-1, 1, order="F")
            ih_term = float((Qf.T @ (_vb_mod._spm_log(Qf) - log_hf[fi])).reshape(-1)[0])
            G[k, :] -= ih_term
            if _probe_parent and k == 0:
                _vb_mod._PROBE_12F_PARENT["ih_term"] = ih_term
                _vb_mod._PROBE_12F_PARENT["G_after_iH"] = float(np.asarray(G[k, 0], dtype=np.float64))
        ...
        def _qj_cached(j: Any) -> list[Any]:
            ...
            if key not in qj_k:
                qj_k[key] = _vb_mod._cell_get_Qj(Qp, j)
            return qj_k[key]

        if _vb_mod._numel(R) > 0:
            q_cells = _vb_mod._cell_get_Qj(Qp, r)
            if _probe_parent and k == 0:
                Rv = np.asarray(R, dtype=np.float64)
                _vb_mod._PROBE_12F_PARENT["R_shape"] = list(Rv.shape)
                _vb_mod._PROBE_12F_PARENT["R_max"] = float(np.max(Rv)) if Rv.size else 0.0
                _vb_mod._PROBE_12F_PARENT["R_sum"] = float(np.sum(Rv))
                _vb_mod._PROBE_12F_PARENT["r_factors"] = np.atleast_1d(np.asarray(r, dtype=np.int64)).ravel().tolist()
                if len(q_cells) == 1:
                ...
                _vb_mod._PROBE_12F_PARENT["R_nz_idx"] = nz[:8].tolist()
                _vb_mod._PROBE_12F_PARENT["Q_at_R_nz"] = Qflat[nz[:8]].tolist() if nz.size else []
                _vb_mod._PROBE_12F_PARENT["dot_manual_RQ"] = float(
                    (Rflat.reshape(1, -1) @ Qflat.reshape(-1, 1)).reshape(-1)[0]
                )
                for fi in _vb_mod._PROBE_12F_PARENT["r_factors"]:
                    Qfi = np.asarray(Qp[int(fi) - 1], dtype=np.float64).ravel(order="F")
                    Pfi = _fwd_col_vec(ws, ws_m, P, mi, int(fi) - 1, t - 1, use_ws=use_ws)
                    _vb_mod._PROBE_12F_PARENT[f"Qf_len_f{fi}"] = int(Qfi.size)
                    _vb_mod._PROBE_12F_PARENT[f"Qf_max_f{fi}"] = float(np.max(Qfi)) if Qfi.size else 0.0
                    _vb_mod._PROBE_12F_PARENT[f"Pf_sum_f{fi}"] = float(np.sum(Pfi))
            g_risk = np.asarray(_forwards_dot_R_qcells(R, q_cells), dtype=np.float64).reshape(-1)
            if _probe_parent and k == 0:
                _vb_mod._PROBE_12F_PARENT["spm_dot_R_Q"] = float(g_risk.reshape(-1)[0]) if g_risk.size else 0.0
                _vb_mod._PROBE_12F_PARENT["G_after_dot"] = float(np.asarray(G[k, 0], dtype=np.float64))
                _vb_mod._PROBE_12F_PARENT["done"] = True
            if g_risk.size == 1:
                G[k, :] += float(g_risk[0])
            elif g_risk.size == Ni:
                G[k, :] += g_risk
        ...
        for i_cov in range(Ni):
            ...
            for ig in np.atleast_1d(np.asarray(gi, dtype=np.int64).ravel()):
                ...
                for g in np.atleast_1d(np.asarray(gg, dtype=np.int64).ravel()):
                    ...
                    No[0, i_cov] += float(
                        np.asarray(
                            _vb_mod._spm_log(np.array([[float(np.size(qo))]], dtype=np.float64)),
                            dtype=np.float64,
                        ).reshape(-1)[0]
                    )
                    G[k, i_cov] -= float((qo.T @ _vb_mod._spm_log(qo)).reshape(-1)[0])
                    Cmg = c_f64[gi0]
                    if Cmg is not None:
                        c_cells = idm.get("C", [])
                        cg = None
                        if isinstance(c_cells, (list, tuple)) and len(c_cells) >= int(g):
                            cg = c_cells[int(g) - 1]
                        if cg is not None and _vb_mod._numel(cg) > 0:
                            fC = int(np.asarray(cg, dtype=np.int64).ravel()[0])
                            U = np.asarray(
                                _forwards_dot_vec_match(_vb_mod._spm_log(Cmg), Qp[int(fC) - 1]),
                                dtype=np.float64,
                            ).reshape(-1, 1, order="F")
                        else:
                            U = np.asarray(_vb_mod._spm_log(Cmg), dtype=np.float64).reshape(
                                -1, 1, order="F"
                            )
                    ...
                    if _vb_mod._numel(pAg) > 0:
                        ...
                        G[k, i_cov] += float((pgl.T @ (_vb_mod._spm_log(pgl) - _vb_mod._spm_log(pal))).reshape(-1)[0])
                    else:
                        Pa[int(g)] = {}
    ...
    if t < N:
        ...
        for k in range(nk):
            ...
            for ii_lin in range(int(q.size)):
                ...
                memo_key = _efe_memo_key(m, t + 1, k, fi, Qp, id_fu_list)
                if memo_key in _efe_memo:
                    E = _efe_memo[memo_key]
                else:
                    for g in ig.tolist():
                        ...
                        for o in np.atleast_1d(np.asarray(gg, dtype=np.int64).ravel()):
                            ...
                            O[mi][int(o) - 1][t] = col
                    for f in range(nf):
                        _fwd_write_belief_col(ws, ws_m, bundle, P, mi, f, t, Qp[f], use_ws=use_ws)
                    if use_ws:
                        E = _forwards_ws(
                            ws,
                            ws_m,
                    ...
                    _efe_memo[memo_key] = np.asarray(E, dtype=np.float64).copy()
