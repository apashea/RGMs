...
from python_src.toolbox.DEM import spm_MDP_VB_XXX as _m_hier
...
def _vb_hierarchical_q_O_ng_t_hstack_optim(
    ...
    if isinstance(old, np.ndarray):
        ...
        old_rows = _m_hier._vb_hierarchical_O_field_to_ng_t_rows(
            old,
            t_child=int(old.shape[1]) if old.ndim >= 2 else 1,
            ng=ng_guess,
        )
        return _vb_hierarchical_q_O_ng_t_hstack_optim(old_rows, new_rows)
    if isinstance(old, list) and old and _m_hier._vb_hierarchical_q_O_is_ng_t_rows(old):
...
def _vb_hierarchical_q_append_level_optim(
    ...
    if ck == "O":
        ...
        no = _m_hier._vb_no_list_from_mdp(child_upd)
        new_rows = _m_hier._vb_hierarchical_O_field_to_ng_t_rows(
            child_upd[ck],
            t_child,
            ng=ng_m,
            no=no,
        ...
        return
    if ck in ("P", "X"):
        new_level = _m_hier._vb_hierarchical_q_field_to_cell_row(child_upd[ck], t_child=t_child, kind=ck)
        if qv[li] is None:
            qv[li] = new_level
    ...
    if ck in ("s", "u", "o"):
        ...
        if isinstance(old, list):
            if old and _m_hier._vb_hierarchical_q_O_is_ng_t_rows(old):
                old_m = _m_hier._vb_hierarchical_q_O_level_to_matrix(
                    old,
        ...
        return
    if ck in ("Y", "j", "i"):
        new_nested = _m_hier._vb_hierarchical_field_to_ot_nested(child_upd[ck], t_child=t_child)
        if qv[li] is None:
            qv[li] = new_nested
        else:
            qv[li] = _m_hier._vb_hierarchical_q_ot_nested_hstack(qv[li], new_nested)
        return
    new_cells = _m_hier._vb_hierarchical_q_field_to_cell_row(child_upd[ck], t_child=t_child, kind=ck)
    if qv[li] is None:
        qv[li] = new_cells
...
def vb_hierarchical_subordinate_outcomes_optim(
    ...
    *,
    ...
    _vb_hierarchical_apply_S_as_O_if_present = _m_hier._vb_hierarchical_apply_S_as_O_if_present
