...
def _orch_assign_o_slot(
    ...
    """Write ``O(m,o,t)`` to bundle + sync ``ws.O`` (**5-C-arena** forwards read path)."""
    bundle["O"][mi][o_idx][t_idx] = col
    ws = bundle.get("_vb_workspace_optim")
    if isinstance(ws, VbWorkspace):
        ws_assign_o_slot(ws, bundle, mi, o_idx, t_idx, col)
...
def _prior_qp_paths_states_one_model(
    ...
    for f_idx in range(nf_gen):
        ...
        Q_prev = ws_q_compact_column(ws, bundle, mi, f_idx, t_idx - 1).copy()
        Bmf = B_t[mi][f_idx]
        if nu_mf > 1:
...
def _gen_control_one_model(mi: int, models: list[dict[str, Any]], bundle: dict[str, Any], t_idx: int) -> None:
    ...
    if float(bundle["process"][mi]) > 0.0:
        ...
        nf = int(bundle["Nf"][mi])
        A_list = bundle["A"][mi]
        Q_slice = [bundle["Q"][mi][f][t_idx] for f in range(nf)]
        nf_gp = len(md["GB"])
            ...
            for o_1based in i_vals:
                ...
                if n_ot > 0:
                    ...
                    if ni == mi:
                        ...
                        _orch_assign_o_slot(
                            bundle, mi, o_idx, t_idx, O_shell[ni][o_idx][t_idx]
                        )
    ...
    if t_m > 1:
        for f_idx in range(nf_m):
            ...
            if nu_mf > 1:
                ...
                Qt = np.asarray(bundle["Q"][mi][f_idx][t_idx], dtype=np.float64).reshape(-1, 1, order="F")
                Qtm1 = np.asarray(bundle["Q"][mi][f_idx][t_idx - 1], dtype=np.float64).reshape(-1, 1, order="F")
                LP = np.asarray(_spm_log(bundle["P"][mi][f_idx][t_idx - 1]), dtype=np.float64).reshape(-1, 1)
