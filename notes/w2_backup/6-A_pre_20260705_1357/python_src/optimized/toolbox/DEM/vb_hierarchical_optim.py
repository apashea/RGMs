...
from python_src.optimized.toolbox.DEM.vb_workspace_optim import ws_q_cell, ws_q_row_at_t, ws_require, ws_assign_o_slot, ws_get
...
def vb_hierarchical_subordinate_outcomes_optim(
    ...
    *,
    ...
    from python_src.optimized.toolbox.DEM.vb_run_arena_optim import arena_child_q_from_parent, arena_get
    ...
    for mm in M_vec:
        ...
        # **5-C-arena:** Q-record copy via arena (pool target); not ``checkX`` (never touches ``Q``).
        if "Q" in parent:
            arena = arena_get(bundle)
            if arena is not None:
                child["Q"] = arena_child_q_from_parent(arena, mi, parent["Q"])
            else:
        ...
        for f in range(len(idD)):
            for g in np.atleast_1d(np.asarray(idD[f], dtype=np.int64).ravel()).tolist():
                ...
                O_shell[mi][gi][t_idx] = col_o
                ws_h = ws_get(bundle)
                if ws_h is not None:
                    ws_assign_o_slot(ws_h, bundle, mi, gi, t_idx, col_o)
