...
**ENDGAME-1 tranche 7-D (2026-07-06):** hoist ``A_sub`` once per ``_spm_VBX_update``; batched ``p_w`` merge.
...
def spm_VBX_optim(O, P, A, id_dict):
    ...
    for p in range(len(ig)):
        for g in np.atleast_1d(np.asarray(ig[p], dtype=int)).ravel().tolist():
            ...
            for q in range(Nq):
                ...
                _, Fq = _spm_VBX_update(
                    Srow, A[gi - 1], O, iq[q], jq[q]
                )
                for jf in j_list:
                    S[q][jf - 1] = Srow[jf - 1]
    ...
    for f in range(Nf):
        slot = P[f]
        base = np.asarray(slot, dtype=float)
        if Nq == 1:
            acc = np.asarray(S[0][f], dtype=float) * float(p_w[0])
        else:
            rows = np.vstack(
                [np.asarray(S[q][f], dtype=float).reshape(1, -1) for q in range(Nq)]
            )
            acc = np.dot(p_w.reshape(1, -1), rows).reshape(base.shape, order="F")
        if base.flags.writeable and acc.shape == base.shape:
            np.copyto(base, acc)
        else:
            P[f] = acc
    return P, matlab_scalar(np.array(float(np.dot(F_col.reshape(-1), p_w.reshape(-1)))))
