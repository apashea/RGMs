...
def _vb_hierarchical_field_to_ot_nested(field: Any, *, t_child: int) -> list[list[Any]]:
    ...
    if isinstance(field[0], (list, tuple)):
        ...
        for o_row in field:
            ...
            out.append(
                [
                    np.asarray(row[t], dtype=np.float64).copy() if t < len(row) and row[t] is not None else None
...
def _vb_hierarchical_q_ot_nested_hstack(
    ...
    if old is None:
        return [[np.asarray(x, dtype=np.float64).copy() if x is not None else None for x in row] for row in new_nested]
