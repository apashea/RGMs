...
See ``XXX_optim.md`` § **XXX_matlab** (**XXX_matlab-1**, **HARD DIRECTIVE**).
Child ``Q`` is **shared** with parent; append after nested return. Do **not** restore
``vb_optim_deepcopy`` on hierarchical ``Q``.

Gate during bring-up: **``3f`` only** after PASS2 contract **1**+**5** landed.
"""
...
def _vb_hierarchical_q_append_level_optim(
    ...
    if ck in ("Y", "j", "i"):
        ...
        return
    # MATLAB ~1241: mdp.Q.E{L} = [mdp.Q.E{L} mdp.F] — numeric concat (mapping E←F).
    # Do not wrap length-n F as one list cell (+1); grow by numel(F) like MATLAB.
    if ck == "F":
        ...
        old = qv[li]
        if isinstance(old, list):
            # Prior wrong packing: one cell per hit → flatten then hstack F samples.
...
def _vb_hierarchical_update_parent_Q_from_child_optim(
    ...
    **XXX_matlab-1:** parent/child share one ``Q``; nested VB does not grow ``Q.E``;
    growth is this post-return append. Prefer ``child_upd["Q"]`` when it is the shared
    dict (same object as ``parent["Q"]``); else append on ``parent["Q"]``.
    """
    ...
    # XXX_matlab-7: post-record F / Q.E sizes (env-gated; no deepcopy)
    if os.getenv("RGMS_XXX_MATLAB_7", "").strip().lower() in ("1", "true", "yes"):
        _xm7_record_post_merge(child_upd, qrec, li, path_hint=path_hint)

    parent["Q"] = qrec
    child_upd["Q"] = qrec


_XM7_ROWS: list[dict[str, Any]] = []


def _xm7_record_post_merge(
    child_upd: dict[str, Any],
    qrec: dict[str, Any],
    li: int,
    *,
    ...
    """Append one post-merge row for XXX_matlab-7 (shared with MATLAB dump schema)."""
    f_arr = np.asarray(child_upd.get("F", 0.0), dtype=np.float64).ravel()
    qe = qrec.get("E", [])
    e_level = None
    if isinstance(qe, list) and li < len(qe):
        e_level = qe[li]
    elif isinstance(qe, np.ndarray):
        e_level = qe
    if e_level is None:
        e_numel = 0
    elif isinstance(e_level, list):
        e_numel = len(e_level)
    else:
        e_numel = int(np.asarray(e_level, dtype=np.float64).size)
    qf = qrec.get("F", float("nan"))
    try:
        qf_f = float(np.sum(np.asarray(qf, dtype=np.float64)))
    except Exception:
        qf_f = float("nan")
    row = {
        "n": len(_XM7_ROWS) + 1,
        "L": li + 1,
        "path": path_hint,
        "F_numel": int(f_arr.size),
        "F_sum": float(np.sum(f_arr)) if f_arr.size else 0.0,
        "QE_L_numel": e_numel,
        "Q_F": qf_f,
        "F_preview": f_arr[:8].tolist(),
    }
    _XM7_ROWS.append(row)
    if row["n"] <= 4 or row["n"] % 32 == 0:
        print(
            f"[XM7-optim] n={row['n']} L={row['L']} path={row['path']} "
            f"F_numel={row['F_numel']} F_sum={row['F_sum']:.6g} "
            f"QE_L_numel={row['QE_L_numel']} Q_F={row['Q_F']:.6g}",
            flush=True,
        )


def xm7_rows_clear() -> None:
    _XM7_ROWS.clear()


def xm7_rows_get() -> list[dict[str, Any]]:
    return list(_XM7_ROWS)
...
def vb_hierarchical_subordinate_outcomes_optim(
    ...
    *,
    ...
    for mm in M_vec:
            ...
            if isinstance(idE, (list, tuple)) and f < len(idE):
                for g in np.atleast_1d(np.asarray(idE[f], dtype=np.int64).ravel()).tolist():
                    j = spm_parents(
                        bundle["id"][mi],
                        int(g),
                        ws_q_row_at_t(ws, mi, t_idx, bundle=bundle),
                    )[0]
                    j_arr = np.atleast_1d(np.asarray(j, dtype=np.int64).ravel())
                    q_list = [ws_q_cell(ws, mi, int(jj) - 1, t_idx, bundle) for jj in j_arr]
        ...
        # **XXX_matlab-1 A:** ``mdp.Q = MDP(m).Q`` — share, do not clone (**HARD DIRECTIVE**).
        if "Q" in parent:
            child["Q"] = parent["Q"]
        ...
        for f in range(len(idE)):
            for g in np.atleast_1d(np.asarray(idE[f], dtype=np.int64).ravel()).tolist():
                ...
                col_o = _vb_o_cell_to_column(Pf[:, -1:], no_g)
                ws_assign_o_slot(ws_h, None, mi, gi, t_idx, col_o)
