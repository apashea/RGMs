...
def _induction_cached(
    ...
    *,
    ...
    """Induction with memo — **ENDGAME-1 t6** narrows cache key to ``hif_*`` columns when static ``full``."""
    key_parts: list[Any] = [int(mi), int(t), int(horizon)]
    if ws is not None and ws_m is not None and ws_t_col is not None:
        if ind_static is not None and ind_static.branch == "full":
            key_factors = ind_static.hif_list if ind_static.has_cid else ind_static.hif_kept
        else:
            key_factors = list(range(int(nf)))
        for f in key_factors:
            fi = int(f) - 1 if ind_static is not None and ind_static.branch == "full" else int(f)
            key_parts.append(ws_q_compact_column(ws, ws_m, fi, ws_t_col, None).tobytes())
        q_pass: list[Any] = []
    else:
        if P_now is None:
