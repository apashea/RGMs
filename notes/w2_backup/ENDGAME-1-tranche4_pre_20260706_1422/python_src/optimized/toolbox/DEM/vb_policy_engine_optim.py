...
**Tranche 3 (2026-07-06):** ws-native induction (no ``P_now`` list); single ``pf_at_t`` read pass;
direct ``policy_engine_step`` EFE subcall (no ``_forwards_compute`` re-entry).
...
def _pf_columns_at_t(
    ...
    *,
    ...
    """One pass — ``P{m,f,t}`` columns as ``(n,1)`` float64 (**ENDGAME-1 t3**)."""
    out: dict[int, np.ndarray] = {}
    if use_ws and ws is not None and ws_m is not None:
        for f in range(nf):
            out[f] = ws_q_compact_column(ws, ws_m, f, t_col, None).reshape(-1, 1, order="F")
    else:
        for f in range(nf):
            out[f] = _fwd_col_mat(ws, ws_m, P, mi, f, t_col, use_ws=use_ws, bundle=bundle)
    return out
...
def policy_engine_step(
    ...
    *,
    ...
    pf_at_t = _pf_columns_at_t(ws, ws_m, P, mi, nf, t_col, use_ws=use_ws, bundle=bundle)

    Qp: list[Any] = [None] * nf
    for f in id_fp_list:
        fi = int(f) - 1
        Bf1 = np.asarray(B_slice[fi][0], dtype=np.float64)
        Qp[fi] = Bf1 @ pf_at_t[fi]
