...
def _ind_backward_paths_into(
    ...
    ind_backward_paths_into(pf_col, Bf_prop, N, I_big, prev_f, next_f)
...
def _ind_run_goal_search(
    *,
    ...
    I_snapshots: list[np.ndarray] = []

    for i in range(nh):
        _ind_backward_paths_into(Pf[:, i], Bf_prop, N, I_big, prev_f, next_f)
        G[:, i] = _ind_g_column(I_big, qf_ravel, ncols)
        I_snapshots.append(I_big.copy())
    ...
    p_vec = I_snapshots[orig_j][:, col_idx].astype(np.float64)
